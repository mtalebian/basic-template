...
const cssColFormLabel = (size) =>
    size === "sm" ? "col-form-label col-form-label-sm" : "col-form-label";
const cssFormLabel = (size) =>
    size === "sm" ? "form-label form-label-sm" : "form-label";
const cssFormSelect = (size) =>
...
function toFormControlClassNames(
    size,
    readonly,
    controlClassName,
    { input, meta }
    ...
    return classNames(
        cssFormControl(size, readonly),
        controlClassName,
        final_from_classes(input, meta)
    );
}

function toSelectControlClassNames(
    size,
    readonly,
    controlClassName,
    { input, meta }
...
export const FinalField = ({
    ...
    function doRenderLabel({ input, meta }) {
        ...
        var cn = classNames(
            labelClassName,
            has_columns ? cnLabelColumn : cssFormLabel(size)
        );
        return !has_columns && !label ? null : (
    ...
    function doRenderInput({ input, meta }) {
        ...
        var args = settings.displayErrorsInTitle
            ? { ...props, title: meta.error }
            : props;
        ...
        var inp = (
            <input
                ...
                {...input}
                {...args}
                data-meta={1 && JSON.stringify(meta, null, 2)}
            />
        );
...
const _Final_Check = ({
    ...
    function doRenderLabel() {
        ...
        return !title ? null : (
            <span className={classNames(labelClassName, "pe-2")}>{title}</span>
        );
    }
